...
```mermaid
graph TD
    A["上一轮输入<br/>(L(i-1), R(i-1))"] --> B["R(i-1)"]
    A --> C["L(i-1)"]
    D["子密钥 K(i)"] --> E["轮函数 F<br/>F(R(i-1),K(i))"]
    B --> E
    E --> G["XOR"]
    C --> G
    G --> H["R(i) = L(i-1) XOR<br/>F(R(i-1), K(i))"]
    B --> I["L(i) = R(i-1)"]
    H --> J["本轮输出<br/>(L(i), R(i))"]
    I --> J
```
...
graph TD
    ...
    LS1L --> LS2L["LS-2<br/>(+1圈)"]:::op
    LS1R --> LS2R["LS-2<br/>(+1圈)"]:::op
...
2. **第一轮 (使用K1)**
   ...
   * **轮函数 f(R0, K1)**：这是S-DES最核心的部分。
     1. **扩展/置换 (E/P)**：**“扩展 P盒” (Expansion P-Box)**，本质是个特殊的P盒子。**目的是将4位的右半部分R0扩展成8位，以便和8位的子密钥进行异或**。其算法也和一般P盒相似，具体的**硬编码映射规则**如下（这就是所谓的“置换表”）：
        ...
        | **输出位索引** | **1** | **2** | **3** | **4** | **5** | **6** | **7** | **8** |
        | -------------------- | ----------- | ----------- | ----------- | ----------- | ----------- | ----------- | ----------- | ----------- |
        | **来源输入位** | **4** | **1** | **2** | **3** | **2** | **3** | **4** | **1** |
...
3. **中间交换 (SW - Switch Function)**
   ...
   **定义**：在 S-DES 标准中，SW 是一个显式的独立部件。它强制交换 $f_{k1}$ 输出的左右两半 $(L_1, R_1)$，使其变为 $(R_1, L_1)$ 以供下一轮处理。
...
2. **第一轮 (使用 K1 \= 10100100\)**
   * **轮函数 f(R0, K1)**:
     ...
     * S盒代换:
       * S0盒输入: 0001 \-\> 行01, 列00 \-\> 查表得3 \-\> 二进制 11
       * S1盒输入: 1010 \-\> 行10, 列01 \-\> 查表得0 \-\> 二进制 00
       * S盒总输出: 1100
