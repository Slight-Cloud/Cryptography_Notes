...
用于**验证信息的来源（身份认证）和保证信息的完整性（防篡改）**。核心思想是“我用我的私钥做个标记，你们用我的公钥来验证这个标记对不对”。
...
2. **加密摘要**：A 使用自己的**私钥**，对上一步生成的“数字指纹”进行加密。
3. **形成签名**：这个**加密后的“数字指纹”**，就是本次操作的**数字签名**。
